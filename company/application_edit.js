...
function del_con()
{
	return confirm("Are you sure delete this information");	
		...
		function cal_back_t()
		{
			...
			var pay_tk=parseFloat("0"+document.getElementById("pay_tk").value);
			document.getElementById("back_tk").value=pay_tk-total_payable_bill;
		}				
...
function print_order(type, act, order)
{
	$.post("return_user.php",{},function(result_u)
	{
		...
		{
			var user=result_u;
			$.post("print_order.php",{order:order, type:type, user:user, act:act},function(result)
			{
				if(result)
				{
					if(type=='food'){var en=3;}else{var en=1}
                                        var i=1;
...
{
	$.post("set_session.php",{},function(h_status)
	{
		if(h_status)
		{
                        $.post("return_user.php",{},function(result_u)
                        {
                                ...
                                {
                                        var user=result_u;
                                        $.post("print_day_report.php",{user:user, terminal:"Terminal 1", h_status:h_status},function(result)
                                        {
                                                if(result)
                                                {
                                                        var popupWin = window.open('', '_blank', 'width=300,height=600');
                                                        ...
                                                        $.post("print_day_report.php",{user:user, terminal:"Terminal 2", h_status:h_status},function(result)
                                                        {
                                                                if(result)
                                                                {
                                                                        var popupWin = window.open('', '_blank', 'width=300,height=600');
...
function order_plus_minus(id,quan, action, order_no, user)
{
    ...
    if(quantity>=0)
    {
        $.post("order_plus_minus.php",{id:id, quantity:quantity},function(result)
        {
                if(result)
                {
                    if(result=='ok')
